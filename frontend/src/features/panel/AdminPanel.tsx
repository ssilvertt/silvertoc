...
import { MeMonitorPanel } from "@/features/me-monitor/MeMonitorPanel"
...
export function AdminPanel({
  ...
  return (
    <>
      <section className="mt-6 grid gap-4 lg:grid-cols-[1fr_2fr]">
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <p>ME предметов: {adminSummary?.monitorItemsTotal ?? "-"}</p>
            <p>ME включено: {adminSummary?.monitorItemsEnabled ?? "-"}</p>
          </CardContent>
        </Card>
      ...
      <MeMonitorPanel
        items={monitorItems}
        onRefresh={onRefreshMonitorItems}
        onCreate={onCreateMonitorItem}
        onUpdate={onUpdateMonitorItem}
        onDelete={onDeleteMonitorItem}
      />
    </>
  )
}
